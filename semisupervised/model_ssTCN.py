...
class AttentionBlock(tf.layers.Layer):
    ...
    def call(self, inputs, out0, training=True):
        ...
        out = tf.reshape(out, [-1, self.seq_len, aa_size])
        out = out + bo #[batch_size, seq_len, aa_size]
        
        return out

    
class DNNBlock(tf.layers.Layer):
    def __init__(self, seq_len, p_keep, name=None):
        super(DNNBlock, self).__init__(name=name)
        self.seq_len = seq_len
        self.p_keep = p_keep
            
    def call(self, X, inputs, training=True): 
        x = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=X, logits=inputs) 
        #[batch_size, seq_len]
        
        def wd(shape):
            in_dim = shape[0]
            xavier_stddev = 1. / tf.sqrt(in_dim / 2.)
            return tf.Variable(tf.random_normal(shape, stddev = xavier_stddev), name='DNN')

        def bd(shape):
            return tf.Variable(tf.constant(0.01, shape=shape), name='DNN')
...
class Network(tf.layers.Layer):
    def __init__(self, seq_len, nchan, levels, kernel_size, p_keep, name=None):
        ...
        self.layers.append(DNNBlock(
            seq_len, p_keep=p_keep, name="tblock_{}".format(self.num_levels)))

    
    def call(self, inputs, training=True, **kwargs):
        d={}
        for i in range(0, self.num_levels + 3): 
            if i == 0: #EmbeddingBlock
                outputs = self.layers[i](inputs, training = training)
                X = inputs
            ...
            elif i == self.num_levels + 2: #DNNBlock
                pred_val = self.layers[i](X, outputs, training = training)
                
            else: #TCBlock
                if i != 2 and i % 2 == 0: #residual connection
                    outputs += d['outputs_%d'%(i-3)]
                outputs = self.layers[i](outputs, training = training)
                d['outputs_%d'%i] = outputs
            
                    
        return outputs, pred_val
